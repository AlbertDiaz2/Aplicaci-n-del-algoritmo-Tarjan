...
public class DigrafoActividad2 implements Grafo
{

    // Atributos de la clase DigradoActividad2.
    private int nFilas;
        ...
        try
        {
            ...
            while((s=in.readLine())!= null)
            {
                ...
                if( contador > 1 && contador < nFilas + 2)
                {
                    ...
                    for( int i = 1; i < columnas + 1; i++ )
                    {
                        edificiosAux[j][i] = Integer.parseInt( tok[i-1]);
                    }
    ...
    public void inicializarEstructuras( int filas, int columnas ) {
        ...
        edificiosAux = new int[filas+2][columnas+2];
    ...
    /**
     * Descripcion: Funcion que vacia la listaDeListas, componentes
     *              , listaDeArcos y time
     * Precondicion: True
     * Postcondicion: reset de las variables
     * Orden: O(Cuadratico)
     */

    public void reset() {

        listaDeListas.removeAll(listaDeListas);

        componentes.removeAll(componentes);

        listaDeArcos.removeAll(listaDeArcos);
        ...
        for( int i = 1; i < nFilas+1; i++ )
        {
            for( int j = 1; j < nColumnas+1; j++ )
            {
                agregarVertice( "["+i+","+j+"]",i, j, 
                                (double) edificios[i][j] );
            }
        }
    }

    /**
     * Descripcion: Funcion que controla la subida del nivel de agua
     * Precondicion: True
     * Postcondicion: Control de subida de agua
     * Orden: O(Cuadratico)
     */
    ...
    private void dfs(Vertice v, int nivel) { 
     ...
     * @param lista: lista de componentes conexas del grafo.
     * Postcondicion: Desagues calculados.
     * Orden: O(Lineal)
     */

    public void calculaDesagues(ArrayList<Vertice> lista, int nivel) {
        ...
        if( !control )
        {
            for( Vertice v: lista )
            {
                ...
                edificios[v.getX()][v.getY()] = nivel;
            }
        }

    }

    public void totalEmpozado() {
        ...
        for(int i = 1; i < nFilas+1; i++)
        {
            for(int j = 1; j < nColumnas+1; j++ )
            {
                if( respuesta[i-1][j-1].equals("x") )
                {
                    total = total + (edificios[i][j] - edificiosAux[i][j]);
                }
            }
        }

        System.out.println("El volumen es igual a "+total+" metros cubicos.");
    }
    ...
    public void mostrarEdificios() {
        ...
        System.out.println( "Matriz de edificios" );
        for( int i = 0; i < nFilas + 2; i++ )
        {
            for( int j = 0; j < nColumnas + 2; j++ )
            {
                System.out.print( edificios[i][j] );
                System.out.print( " \t" );
            }
    ...
    public void mostrarEdificiosAux() {

        System.out.println( "Matriz de edificiosAux" );
        for( int i = 0; i < nFilas + 2; i++ )
        {
            for( int j = 0; j < nColumnas + 2; j++ )
            {
                System.out.print( edificiosAux[i][j] );
                System.out.print( " \t" );
            }
...
} // Fin de la clase DigrafoActividad2